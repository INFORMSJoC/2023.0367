...
parameter BDsol(iter, hubs);
parameter totalBDsol(hubs);
totalBDsol(hubs) = 0;
parameter maxsolnum;
set Qdivision /1*2/;
parameter Qsizedivision;
alias (Qdivision, Qdivision1);
set QhubDiv(Qdivision, hubs);
parameter predivsize;
parameter maxhubnum;
...
if(ord(iter) eq 1,
      ...
      abort$(master_iter1.modelstat=4) "Relaxed Master is infeasible";
                  ...
                  if(UB - LB le avaerageHubCost,
                           loop(testiter$(totaltime le maxtime and testingtime lt maxtime/2),
                                    testdone = 0 ;
                                    Qhubs(hubs) = no;
                                    ...
                                    Qsizedivision(Qdivision) = floor(getQhubsize/card(Qdivision));
                                    Qsizedivision(Qdivision)$(ord(Qdivision) eq card(Qdivision)) = getQhubsize - sum(Qdivision1$(ord(Qdivision1) lt card(Qdivision1)), Qsizedivision(Qdivision1));

                                    QhubDiv(Qdivision, hubs) = no;

                                    getQhubsize = 0;
                                    maxhubnum = 0;
                                    loop(Qdivision,
                                             predivsize = sum(Qdivision1$(ord(Qdivision1) le ord(Qdivision)), Qsizedivision(Qdivision1));
                                             loop(hubs$(Qhubs(hubs) and getQhubsize lt predivsize and ord(hubs) gt maxhubnum),
                                                      getQhubsize = getQhubsize + 1 ;
                                                      QhubDiv(Qdivision, hubs) = yes;
                                                      maxhubnum = ord(hubs);
                                                  );
                                         );

                                    loop(Qdivision$(totaltime le maxtime and testingtime lt maxtime/2),
                                                Qhubs(hubs) = QhubDiv(Qdivision, hubs);
                                                getQhubsize = 0;
                                                loop(hubs$Qhubs(hubs),
                                                         getQhubsize = getQhubsize + 1 ;
                                                     );
                                                removesizefromQ = floor(0.25*getQhubsize);
                                                if(ord(testiter) eq 1,
                                                         foundhubs(hubs) = no;
                                                         findlimitnum = 0;
                                                         loop(findlimit$(findlimitnum le removesizefromQ),
                                                                  minhubnum = smin(hubs$((not foundhubs(hubs)) and Qhubs(hubs)), rankhubcost(hubs));
                                                                  foundflag = 0;
                                                                  loop(hubs$((not foundhubs(hubs)) and Qhubs(hubs) and minhubnum eq rankhubcost(hubs) and foundflag eq 0),
                                                                            findlimitnum = findlimitnum + 1 ;
                                                                            Qhubs(hubs) = no ;
                                                                            foundflag = 1;
                                                                       );
                                                               ...
                                                               findlimitnum = 0;
                                                               loop(findlimit$(findlimitnum le removesizefromQ),
                                                                        maxsolnum = smax(hubs$((not foundhubs(hubs)) and Qhubs(hubs)), totalBDsol(hubs));
                                                                        foundflag = 0;
                                                                        loop(hubs$((not foundhubs(hubs)) and Qhubs(hubs) and maxsolnum eq totalBDsol(hubs) and foundflag eq 0),
                                                                                  findlimitnum = findlimitnum + 1 ;
                                                                                  Qhubs(hubs) = no ;
                                                                                  foundflag = 1;
                                                                             );
                                                ...
                                                loop(testloop$(testdone eq 0 and availablehub eq 1 and totaltime le maxtime and testingtime lt maxtime/2),
         ...
         ysol(origins, products) = y.l(origins, products);
         usol(destinations, products) = u.l(destinations, products);
         BDsol(iter, hubs) = zsol(hubs);
         totalBDsol(hubs) = totalBDsol(hubs) + BDsol(iter, hubs) ;
    );
*
...
file LagBD_IC_MC_Gita2 /LagBD_IC_MC_Gita2.dat/;
put LagBD_IC_MC_Gita2;
LagBD_IC_MC_Gita2.ap = 1;
