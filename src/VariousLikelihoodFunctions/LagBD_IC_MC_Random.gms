      ...
      abort$(master_iter1.modelstat=4) "Relaxed Master is infeasible";
                  ...
                  loop(hubs$(not removedhubs(hubs) and LPsolz(hubs) eq 0),
                                 if(LPobj + LPRcstz(hubs) gt UB,
                                   ...
                                   if(CACapCoef eq 0.7,
                                         dependvalue(hubs) = -7.0824 + 10.6228*zrlxsol(hubs) + 4.1788*z_feasol(hubs) - 4.843*rankhubcstpct(hubs) ;
                                      );
                                   if(CACapCoef eq 1.0,
                                         dependvalue(hubs) = -7.6776 + 1.3805*zsollogistic(hubs) + 5.8551*zrlxsol(hubs) + 4.9018*z_feasol(hubs) - 2.8239*rankhubRDcstpct(hubs) ;
                                      );
                                   if(CACapCoef eq 1.1,
                                         dependvalue(hubs) = -6.6487 + 0.9176*zsollogistic(hubs) + 14.6113*zrlxsol(hubs) - 3.4590*rankhubcstpct(hubs) ;
                                      );
                  probequalone(hubs) = uniform(0,1);
...
file LagBD_IC_MC_Random05 /LagBD_IC_MC_Random05.dat/;
put LagBD_IC_MC_Random05;
LagBD_IC_MC_Random05.ap = 1;
