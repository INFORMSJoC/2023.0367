...
display z.l ;
parameter zrlxsol2(hubs);
zrlxsol2(hubs) = z.l(hubs) + 0.000001;
...
if(CACapCoef eq 0.7,
      EmbeddedCode Python:
         loaded_model = joblib.load('rf_tuned_model_cap_07.pkl')
      endEmbeddedCode
   );
if(CACapCoef eq 1.0,
      EmbeddedCode Python:
         loaded_model = joblib.load('rf_tuned_model_cap_10.pkl')
      endEmbeddedCode
   );
if(CACapCoef eq 1.1,
      EmbeddedCode Python:
         loaded_model = joblib.load('rf_tuned_model_cap_11.pkl')
      endEmbeddedCode
   );

parameter zsol2(hubs), z_feasol2(hubs), rankhubRDcstpct2(hubs), rankhubcstpct2(hubs) ;
      ...
      abort$(master_iter1.modelstat=4) "Relaxed Master is infeasible";
                  ...
                  loop(hubs$(not removedhubs(hubs) and LPsolz(hubs) eq 0),
                                 if(LPobj + LPRcstz(hubs) gt UB,
                                   ...
                                   zsol2(hubs) = zsol(hubs) + 0.0000001;
                                   z_feasol2(hubs) = z_feasol(hubs) + 0.0000001;
                                   rankhubRDcstpct2(hubs) = rankhubRDcstpct(hubs) + 0.0000001;
                                   rankhubcstpct2(hubs) = rankhubcstpct(hubs) + 0.0000001;
                                   if(CACapCoef eq 0.7,
                                             EmbeddedCode Python:
                                                      df1 = pd.DataFrame(list(gams.get('zrlxsol2')), columns=['hubs', 'LPSol'])
                                                      df2 = pd.DataFrame(list(gams.get('z_feasol2')), columns=['hubs', 'LagSol'])
                                                      df3 = pd.DataFrame(list(gams.get('rankhubcstpct2')), columns=['hubs', 'C_rank_pct_round'])
                                                      X1 = df1[['LPSol']]
                                                      X2 = df2[['LagSol']]
                                                      X3 = df3[['C_rank_pct_round']]
                                                      X = pd.concat([X1, X2, X3], axis=1)
                                                      y_pred_proba_loaded = loaded_model.predict_proba(X)
                                                      y_pred_proba_df_loaded = pd.DataFrame(y_pred_proba_loaded, columns=['Prob_class_0', 'Prob_class_1'])
                                                      prob_class_l_list = y_pred_proba_df_loaded['Prob_class_1'].tolist()
                                                      length_of_list = len(prob_class_l_list)
                                                      index_list = list(range(1, length_of_list + 1))
                                                      tuple_list = list(zip(index_list, prob_class_l_list))
                                                      gams.set('probequalone', tuple_list)
                                             endEmbeddedCode probequalone
                                      );
                                   if(CACapCoef eq 1.0,
                                             EmbeddedCode Python:
                                                      df1 = pd.DataFrame(list(gams.get('zrlxsol2')), columns=['hubs', 'LPSol'])
                                                      df2 = pd.DataFrame(list(gams.get('z_feasol2')), columns=['hubs', 'LagSol'])
                                                      df3 = pd.DataFrame(list(gams.get('zsol2')), columns=['hubs', 'BDSol'])
                                                      df4 = pd.DataFrame(list(gams.get('rankhubRDcstpct2')), columns=['hubs', 'RC_rank_pct_round'])
                                                      X1 = df1[['LPSol']]
                                                      X2 = df2[['LagSol']]
                                                      X3 = df3[['BDSol']]
                                                      X4 = df4[['RC_rank_pct_round']]
                                   ...
                                   if(CACapCoef eq 1.1,
                                             EmbeddedCode Python:
                                                      df1 = pd.DataFrame(list(gams.get('zrlxsol2')), columns=['hubs', 'LPSol'])
                                                      df2 = pd.DataFrame(list(gams.get('zsol2')), columns=['hubs', 'BDSol'])
                                                      df3 = pd.DataFrame(list(gams.get('rankhubcstpct2')), columns=['hubs', 'C_rank_pct_round'])
                                                      X1 = df1[['LPSol']]
                                                      X2 = df2[['BDSol']]
                                                      X3 = df3[['C_rank_pct_round']]
...
file LagBD_IC_MC_RFDT05 /LagBD_IC_MC_RFDT05.dat/;
put LagBD_IC_MC_RFDT05;
LagBD_IC_MC_RFDT05.ap = 1;
