...
master_obj..  MasterCost =e= sum((k), zetavar(k)) + sum((hubs), HubCost(hubs)*z(hubs))
                                 + sum((origins, products)$ProdOriginsset(products, origins), ocCost(origins, products)*y(origins, products))
                                 + sum((destinations, products)$ProdDestinationsset(products, destinations), dcCost(destinations, products)*u(destinations, products)) ;

cut(cutset, k)..
              zetavar(k) =g= sum(products$(product_k_final(products, k)), cutconst1(cutset, products))
                                 - sum((hubs, products)$(product_k_final(products, k)), z(hubs)*cutcoeff1(cutset, hubs, products))
      ...
      abort$(master_iter1.modelstat=4) "Relaxed Master is infeasible";
                  ...
                  loop(hubs$(not removedhubs(hubs) and LPsolz(hubs) eq 0),
                                 if(LPobj + LPRcstz(hubs) gt UB,
                                   ...
                                   if(CACapCoef eq 0.7,
                                         dependvalue(hubs) = -6.6998 + 0.5319*zsollogistic(hubs) + 11.3687*zrlxsol(hubs) + 3.9827*LagZS(hubs) - 1.2935*rankhubRDcstpct(hubs) - 5.4678*rankhubcstpct(hubs) ;
                                      );
                                   if(CACapCoef eq 1.0,
                                         dependvalue(hubs) = -6.6516 - 1.7278*zsollogistic(hubs) + 6.5971*zrlxsol(hubs) + 5.6095*LagZS(hubs) - 7.2122*rankhubRDcstpct(hubs) ;
                                      );
                                   if(CACapCoef eq 1.1,
                                         dependvalue(hubs) = -7.7763 + 0.9777*zsollogistic(hubs) + 10.4872*zrlxsol(hubs) + 3.5981*LagZS(hubs) - 4.2668*rankhubcstpct(hubs) ;
                                      );
                  probequalone(hubs) = exp(dependvalue(hubs))/(1 + exp(dependvalue(hubs))) ;
                  if(UB - LB le avaerageHubCost and HubCostcoef ge 10,
                           getremovedsize = 1;
                           loop(testiter$(getremovedsize gt 0 and totaltime le maxtime),
                                    testdone = 0 ;
                                    Qhubs(hubs) = no;
                                    ...
                                    if(getQhubsize le 0.3*card(hubs),
                                            removesizefromQ = floor(0.25*getQhubsize);
                                             ...
                                             removesizefromQ = 0 ;
                                             loop(hubs$(Qhubs(hubs) and probequalone(hubs) ge 0.3),
                                    ...
                                    getremovedsize = 0;
                                    loop(testloop$(testdone eq 0 and availablehub eq 1 and totaltime le maxtime),
...
file LagBD_IC_MC_ET /LagBD_IC_MC_ET.dat/;
put LagBD_IC_MC_ET;
LagBD_IC_MC_ET.ap = 1;
...
display LagTime, LagLB, LagUB, subprobtime, mastersoltime, testingtime, uncaptime, totaltime, Bendcutnum, totalopenhubs, totalhubcost, totalIassCost, totalDassCost, totaltransportcost, LB, UB;
display removedhubnum;
